...
/// shearing two stacked vector panels in opposite directions. Millie draws the
...
struct MorphingFolderIcon: View {
    ...
    /// Representative site favicon for the folder's contents. Shown inside the
    /// pocket when the folder has no custom symbol, so a folder reads as the
    /// site(s) it holds (e.g. a Grafana folder shows the Grafana mark).
    var faviconIcon: String? = nil
    var faviconPage: String? = nil
    ...
    var body: some View {
        ZStack {
            ...
            ZStack {
                ...
                // Inner content ↔ dots cross-fade. A custom symbol wins; else we
                // show the contents' site favicon; else (no favicon yet) the
                // pocket stays empty. The default "folder" symbol is never drawn
                // inside — that would be a folder inside a folder.
                if hasCustomGlyph {
                    Icon(name: symbol, size: size * 12 / 32)
                        .foregroundStyle(glyphColor)
                        .position(x: center.x, y: center.y)
                } else if showsFavicon {
                    Favicon(icon: faviconIcon, page: faviconPage,
                            image: faviconImage, size: size * 14 / 32)
                        .position(x: center.x, y: center.y)
                }

                // Dots only stand in as the active-tab cue when the pocket has
                // no icon of its own; a folder with a favicon/custom glyph keeps
                // showing that icon even when collapsed with the active tab.
                DotsShape()
                    .fill(stroke)
                    .opacity((showsDots && !hasCustomGlyph && !showsFavicon) ? 1 : 0)
            }
            .modifier(FolderSplit(progress: progress, angleDegrees: -16,
                                  dx: 8, dy: 2, size: size))
        }
    ...
    /// only then do we draw the SF symbol inside the pocket.
    private var hasCustomGlyph: Bool {
        symbol != "folder" && symbol != "folder.fill"
    }

    /// Show a site favicon in the pocket when there's no custom symbol and the
    /// folder gave us something to render (a decoded image, an icon URL, or a
    /// page whose host has a curated brand glyph).
    private var showsFavicon: Bool {
        faviconImage != nil || faviconIcon != nil || faviconPage != nil
    }
